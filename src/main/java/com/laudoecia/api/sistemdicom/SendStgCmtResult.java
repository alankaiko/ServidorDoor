...
class SendStgCmtResult implements Runnable {
	...
	public SendStgCmtResult(Association as, Attributes eventInfo, boolean sameAssoc, Connection remoteConn) {
		this.as = as;
	...
	public void run() {
		...
		try {
			Association diffAssoc = as.getApplicationEntity().connect(as.getConnection(), remoteConn, makeAAssociateRQ());
	...
	private void neventReport(Association as) throws IOException, InterruptedException {
		as.neventReport(UID.StorageCommitmentPushModelSOPClass, UID.StorageCommitmentPushModelSOPInstance, eventTypeId(eventInfo), eventInfo, null).next();
	}
	...
	private AAssociateRQ makeAAssociateRQ() {
		...
		TransferCapability tc = ae.getTransferCapabilityFor(UID.StorageCommitmentPushModelSOPClass, TransferCapability.Role.SCP);
		aarq.addPresentationContext(new PresentationContext(1, UID.StorageCommitmentPushModelSOPClass, tc.getTransferSyntaxes()));
