...
public class UtilsDecm {
	...
	public static String digestAttributes(Attributes attrs, MessageDigest digest) throws IOException {
		...
		DicomOutputStream dout = new DicomOutputStream(nulloutputstream, UID.ExplicitVRLittleEndian);
	...
	public static void setStudyQueryAttributes(Attributes attrs, int numberOfStudyRelatedSeries,
			int numberOfStudyRelatedInstances, String modalitiesInStudy, String sopClassesInStudy,
			int numberVisibleInstances, PrivateTag numberVisibleInstancesTag, Date lastUpdateTime,
		...
		attrs.setString(Tag.SOPClassesInStudy, VR.CS, StringUtils.split(sopClassesInStudy, '\\'));
		if (lastUpdateTimeTag != null && lastUpdateTime != null)
			attrs.setDate(lastUpdateTimeTag.getCreator(), lastUpdateTimeTag.getIntTag(), VR.DT, lastUpdateTime);
		if (numberVisibleInstancesTag != null)
			attrs.setInt(numberVisibleInstancesTag.getCreator(), numberVisibleInstancesTag.getIntTag(), VR.IS,numberVisibleInstances);
	}

	public static void setSeriesQueryAttributes(Attributes attrs, int numberOfSeriesRelatedInstances, int numberVisibleInstances, PrivateTag numberVisibleInstancesTag, Date lastUpdateTime, PrivateTag lastUpdateTimeTag) {
	...
	public static void setRetrieveAET(Attributes attrs, String retrieveAETs) {
		if (retrieveAETs != null)
			attrs.setString(Tag.RetrieveAETitle, VR.AE, StringUtils.split(retrieveAETs, '\\'));
	}
	...
	public static Attributes mergeAndNormalize(Attributes... attrsList) {
				...
				if (!cs.equals(globalCs)) {
					if (!(globalCs.containsASCII() && cs.isASCII())) {
						if (globalCs.isUTF8()) {
							...
							globalCs = SpecificCharacterSet.valueOf("ISO_IR 192"); // UTF-8
							convertToUTF8(attrsList);
							break;
						}
					}
		...
		mergedAttrs.setString(Tag.SpecificCharacterSet, VR.CS, globalCs.toCodes());
	 ...
	 * attributes, according to the configured strategy. Eventually stores in a
	 * modified set all the current modified attributes. A default value for the
	 * update strategy is also passed to the method
	 *
	 * @return <tt>true</tt> if one ore more attribute of the "current" set were
	 *         added or overwritten with a different value
	 */
	public static boolean updateAttributes(Attributes current, Attributes incoming, Attributes modified, AttributeFilter filter, MetadataUpdateStrategy defaultStrategy) {

		MetadataUpdateStrategy strategy = filter.getMetadataUpdateStrategy() != null? filter.getMetadataUpdateStrategy() : defaultStrategy;
